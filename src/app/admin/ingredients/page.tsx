...
const IngredientManagement = () => {
  ...
  const [filteredIngredients, setFilteredIngredients] = useState<Array<{ id: number; name: string }>>([]);
  const [suggestions, setSuggestions] = useState<Array<{ id: number; name: string }>>([]);
  const [loading, setLoading] = useState(true);
  const [newIngredient, setNewIngredient] = useState<{ name: string }>({ name: '' });
  ...
  const handleAddIngredient = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setIsAdding(false);
        setHighlightedIngredientId(addedIngredient.id);
        fetchIngredients();
        setTimeout(() => setHighlightedIngredientId(null), 700);
      } else {
        console.error("Failed to add ingredient:", await response.json());
      }
    } catch (error) {
  ...
  const handleSaveEditIngredient = async () => {
    try {
      const response = await fetch('/api/ingredients', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editIngredientId, ...newIngredient }),
      });
      if (response.ok) {
        setEditIngredientId(null);
        setNewIngredient({ name: '' });
        setHighlightedIngredientId(editIngredientId);
        fetchIngredients();
        setTimeout(() => setHighlightedIngredientId(null), 700);
      } else {
        console.error("Failed to update ingredient:", await response.json());
      }
    } catch (error) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (response.ok) {
            fetchIngredients();
            Swal.fire('Deleted!', 'The ingredient has been deleted successfully.', 'success');
          } else {
            Swal.fire('Failed!', 'There was a problem deleting the ingredient.', 'error');
          }
        } catch (error) {
  ...
  const handleSuggestionClick = (suggestionName: string) => {
    ...
    const filtered = ingredients.filter((ingredient) =>
      ingredient.name.toLowerCase().includes(suggestionName.toLowerCase())
    );
  ...
  const handleEditIngredient = (id: number) => {
    ...
    if (ingredientToEdit) {
      setNewIngredient({ name: ingredientToEdit.name });
      ...
      <Layout>
        <div className="flex flex-col p-4 mt-2 mx-auto max-w-7xl transition-all duration-300 sm:p-6 lg:p-8">
          <div className="flex flex-col items-center sm:items-start space-y-4 sm:flex-row justify-between sm:space-y-0 w-full">
            <h1 className="text-xl sm:text-2xl font-bold">Ingredient Management</h1>
            <button
            ...
            {filteredIngredients.map((ingredient) => (
              <div
                ...
                className={`bg-white p-4 rounded-none border border-black max-w-md w-full flex items-center space-x-6 relative transition-all duration-300 transform hover:scale-105 ${
                  editIngredientId === ingredient.id ? 'h-40' : 'h-24'
                } ${highlightedIngredientId === ingredient.id ? 'bg-yellow-100' : ''}`} // Highlight styling
              >
                  ...
                  {editIngredientId === ingredient.id ? (
                    <>
                      ...
                      <div className="flex space-x-2">
                        <button onClick={handleSaveEditIngredient} className="px-2 py-1 bg-black text-white rounded hover:scale-105 transition-all duration-300">Save</button>
                        <button onClick={handleCancelEditIngredient} className="px-2 py-1 bg-gray-300 text-black rounded hover:bg-gray-400 transition-all duration-300">Cancel</button>
                      </div>
                    </>
                  ) : (
                    <h2 className="text-lg font-semibold transition-all duration-300">{ingredient.name}</h2>
                  )}
                </div>
                <div className="absolute top-2 right-2 flex space-x-2 transition-all duration-300">
                  ...
                  <button onClick={() => handleDeleteIngredient(ingredient.id)} className="text-red-500 hover:text-red-700 transition-all duration-300 transform hover:scale-110">
